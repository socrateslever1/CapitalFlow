  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="space-y-4">
        <textarea placeholder="Descrição da garantia..." value={formData.guaranteeDescription || ''} onChange={e => setFormData({...formData, guaranteeDescription: e.target.value})} className="w-full bg-slate-950/50 border border-slate-800/80 rounded-lg px-5 py-4 text-white min-h-[100px] text-xs resize-none outline-none focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/10 transition-all" />
        ...
        <div className="bg-slate-950/50 border border-slate-800/80 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
                <button type="button" onClick={() => startCamera('guarantee')} className="flex items-center justify-center gap-2 bg-slate-800/50 border border-slate-700/50 p-3 rounded-lg text-[9px] font-black uppercase text-slate-400 hover:text-white border-dashed hover:bg-slate-800 hover:border-slate-600 transition-all"><Camera size={14}/> Foto Garantia</button>
            ...
            <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1 custom-scrollbar">
                {customDocuments.map((doc, i) => (
                    <div key={doc.id} className="flex justify-between items-center bg-slate-900/50 border border-slate-800/50 p-2 rounded-lg">
                ...
                {attachments.map((img, i) => (
                    <div key={`att-${i}`} className="flex justify-between items-center bg-slate-900/50 border border-slate-800/50 p-2 rounded-lg">
