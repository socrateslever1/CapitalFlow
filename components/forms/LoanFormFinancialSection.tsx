        ...
        {(isCardSource || isInstallmentFixed) && (
            <div className="bg-rose-950/20 border border-rose-500/30 p-4 rounded-2xl space-y-4 animate-in slide-in-from-right">
                <div className="flex items-center gap-2 text-rose-400 mb-2">
                    <CreditCard size={16}/>
                    <span className="text-[10px] font-black uppercase tracking-widest">Custo de Captação / Banco</span>
                </div>

                {isInstallmentFixed && (
                    <div className="space-y-4">
                        <div className="flex bg-slate-950/50 p-1 rounded-xl border border-rose-500/20">
                            <button type="button" onClick={() => setFormData({...formData, fundingCalculationMode: 'TOTAL'})} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase ${formData.fundingCalculationMode !== 'RATE' ? 'bg-rose-600 text-white' : 'text-slate-400'}`}>Valor final</button>
                            <button type="button" onClick={() => setFormData({...formData, fundingCalculationMode: 'RATE'})} className={`flex-1 py-2 rounded-lg text-[9px] font-black uppercase ${formData.fundingCalculationMode === 'RATE' ? 'bg-rose-600 text-white' : 'text-slate-400'}`}>Taxa mensal</button>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[9px] text-rose-300/70 font-black uppercase ml-2">Parcelas do Banco</label>
                                <input type="number" min="1" step="1" value={formData.fundingInstallmentsCount || ''} onChange={e => setFormData({...formData, fundingInstallmentsCount: cleanNumberStr(e.target.value)})} className="w-full bg-slate-900 border border-rose-500/30 rounded-2xl px-5 py-4 text-white font-bold focus:border-rose-500 outline-none" />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[9px] text-rose-300/70 font-black uppercase ml-2">Margem Cliente (%)</label>
                                <input type="number" step="0.01" value={formData.customerMarginPercent || ''} onChange={e => setFormData({...formData, customerMarginPercent: cleanNumberStr(e.target.value)})} className="w-full bg-slate-900 border border-emerald-500/30 rounded-2xl px-5 py-4 text-white font-bold focus:border-emerald-500 outline-none" />
                            </div>
                        </div>

                        {formData.fundingCalculationMode === 'RATE' && (
                            <div className="space-y-1">
                                <label className="text-[9px] text-rose-300/70 font-black uppercase ml-2">Juros Banco (% ao mês)</label>
                                <input type="number" step="0.01" placeholder="Ex: 4.49" value={formData.fundingMonthlyRate || ''} onChange={e => setFormData({...formData, fundingMonthlyRate: cleanNumberStr(e.target.value)})} className="w-full bg-slate-900 border border-rose-500/30 rounded-2xl px-5 py-4 text-white font-bold focus:border-rose-500 outline-none" />
                            </div>
                        )}
                    </div>
                )}

                {(isCardSource || formData.fundingCalculationMode !== 'RATE') && <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                ...
                {isInstallmentFixed && (
                    <div className="grid grid-cols-2 gap-3 text-[10px] font-bold">
                        <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-3 text-slate-300">Parcela banco: <span className="text-white">{formatMoney(fixedInstallmentDisplay.bankInstallment)}</span></div>
                        <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-3 text-slate-300">Parcela cliente: <span className="text-emerald-400">{formatMoney(fixedInstallmentDisplay.customerInstallment)}</span></div>
                        <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-3 text-slate-300">Total cliente: <span className="text-white">{formatMoney(fixedInstallmentDisplay.customerTotal)}</span></div>
                        <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-3 text-slate-300">Lucro bruto: <span className="text-emerald-400">{formatMoney(fixedInstallmentDisplay.profit)}</span></div>
                    </div>
                )}

                {isCardSource && <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
