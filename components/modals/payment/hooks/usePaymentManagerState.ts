...
export type InterestHandling = 'CAPITALIZE' | 'KEEP_PENDING' | 'RENEW_KEEP_PENDING';
...
export const usePaymentManagerState = ({ data, paymentType, setPaymentType, avAmount, setAvAmount }: UsePaymentManagerProps) => {
    ...
    const [interestHandling, setInterestHandling] = useState<InterestHandling>('KEEP_PENDING');
    ...
    const debtBreakdown = useMemo(() => {
        ...
        let finalFine = freshCalc.finePart || 0;
        let finalMora = freshCalc.moraPart || 0;
        if (finalFine === 0 && finalMora === 0 && freshCalc.lateFee > 0) finalFine = freshCalc.lateFee;
        if (forgivenessMode === 'CAPITAL_ONLY' || forgivenessMode === 'TOTAL_CHARGES') {
            freshCalc.interest = 0; finalFine = 0; finalMora = 0;
        } else if (forgivenessMode === 'FINE_ONLY') finalFine = 0;
        else if (forgivenessMode === 'MORA_ONLY' || forgivenessMode === 'INTEREST_ONLY') finalMora = 0;
        else if (forgivenessMode === 'FINE_AND_MORA' || forgivenessMode === 'BOTH') { finalFine = 0; finalMora = 0; }
        ...
        return {
            ...
            fine: finalFine,
            dailyMora: finalMora,
            total: (freshCalc.principal || 0) + (freshCalc.interest || 0) + finalFine + finalMora
        };
    ...
    const virtualSchedule = useMemo(() => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (i === 0) {
                ...
                if (isPartial) { label += ` • Restam ${formatMoney(currentInterestDebt)}`; status = 'PARTIAL'; }
            } else if (i === 1) {
                if (realDaysLate > 0) { label = `[${realDaysLate} Dias Atraso]`; status = 'LATE'; }
                else { label = `[em aberto]`; status = 'FUTURE'; }
            } else { label = `[em aberto]`; status = 'FUTURE'; }
            schedule.push({ dateStr, label, fullDate, status, originalDate: date });
        }
        return schedule;
    ...
    useEffect(() => {
        ...
        } else setPaymentType(paymentModalityDispatcher.getConfig(data.loan).defaultAction);
        setCustomAmount('');
