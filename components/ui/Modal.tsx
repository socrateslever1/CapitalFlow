...
export const Modal: React.FC<{onClose: () => void, title: string, children: React.ReactNode}> = ({onClose, title, children}) => (
  <div className="fixed inset-0 z-[var(--z-modal)] bg-slate-950/80 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in duration-300">
    <div className="bg-slate-900/90 border border-slate-800/50 w-full max-w-2xl rounded-lg p-6 sm:p-10 shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 relative flex flex-col max-h-[85dvh] overflow-hidden backdrop-blur-md">
