...
export const SmartSidebar: React.FC<SmartSidebarProps> = ({ events, onAction, currentDate }) => {
    ...
    const lateEvents = events.filter(e => e.status === 'LATE' || e.status === 'OVERDUE');
    const todayEvents = events.filter(e => {
        const d = parseDateOnlyUTC(e.start_time);
        return d.getTime() === today.getTime() && e.priority !== 'URGENT' && e.status !== 'LATE' && e.status !== 'OVERDUE';
    });

    const renderCard = (e: CalendarEvent, badge?: string) => (
        <div key={e.id} onClick={() => onAction(e)} className={`p-3 rounded-lg border mb-2 cursor-pointer transition-all active:scale-95 group ${e.priority === 'URGENT' ? 'bg-emerald-950/30 border-emerald-500/50 hover:bg-emerald-900/40' : (e.status === 'LATE' || e.status === 'OVERDUE') ? 'bg-rose-950/20 border-rose-500/30 hover:bg-rose-900/30' : 'bg-slate-900 border-slate-800 hover:border-blue-500'}`}>
            ...
            <div className="flex items-center gap-1 mt-2 text-[9px] font-bold uppercase text-slate-500">
                ...
                <span>{formatBRDate(e.start_time)}</span>
            </div>
        </div>
    );
