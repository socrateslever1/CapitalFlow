...
export const ExtratoCards = ({ dre, variations, activeFilter, onFilterChange }: { dre: any, variations: any, activeFilter: FilterType, onFilterChange: (f: FilterType) => void }) => {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 shrink-0">
            <div onClick={() => onFilterChange('RECEITA')} className={`cursor-pointer transition-all p-3 rounded-lg border ${getCardStyle('RECEITA')} ${activeFilter !== 'ALL' && activeFilter !== 'RECEITA' ? 'opacity-60' : ''}`}>
            ...
            <div onClick={() => onFilterChange('RECUPERACAO')} className={`cursor-pointer transition-all p-3 rounded-lg border ${getCardStyle('RECUPERACAO')} ${activeFilter !== 'ALL' && activeFilter !== 'RECUPERACAO' ? 'opacity-60' : ''}`}>
            ...
            <div onClick={() => onFilterChange('APORTE')} className={`cursor-pointer transition-all p-3 rounded-lg border ${getCardStyle('APORTE')} ${activeFilter !== 'ALL' && activeFilter !== 'APORTE' ? 'opacity-60' : ''}`}>
            ...
            <div onClick={() => onFilterChange('CAIXA')} className={`cursor-pointer transition-all p-3 rounded-lg border ${getCardStyle('CAIXA')} ${activeFilter !== 'ALL' && activeFilter !== 'CAIXA' ? 'opacity-60' : ''}`}>
