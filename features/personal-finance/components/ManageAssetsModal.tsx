                ...
                {tab === 'ACCOUNTS' && (
                    <div className="space-y-4 animate-in slide-in-from-right">
                        <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 space-y-3">
                            ...
                            <input type="text" placeholder="Nome (Ex: Banco X, Cofre)" className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-xs outline-none" value={accForm.nome} onChange={e => setAccForm({...accForm, nome: e.target.value})} />
                            <input type="text" placeholder="Saldo Inicial (R$)" className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-xs outline-none" value={accForm.saldo} onChange={e => setAccForm({...accForm, saldo: e.target.value})} />
                            <button onClick={handleAddAccount} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-black uppercase text-xs">Salvar Conta</button>
                        </div>

                        <div className="space-y-2">
                            {accounts.map(acc => (
                                <div key={acc.id} className="flex justify-between items-center bg-slate-900 p-3 rounded-lg border border-slate-800">
                                    <div>
                ...
                {tab === 'CARDS' && (
                    <div className="space-y-4 animate-in slide-in-from-right">
                        <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 space-y-3">
                            ...
                            <input type="text" placeholder="Nome (Ex: Nubank Final 1234)" className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-xs outline-none" value={cardForm.nome} onChange={e => setCardForm({...cardForm, nome: e.target.value})} />
                            <input type="text" placeholder="Limite (R$)" className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-xs outline-none" value={cardForm.limite} onChange={e => setCardForm({...cardForm, limite: e.target.value})} />
                            <div className="grid grid-cols-2 gap-2">
                                <input type="number" placeholder="Dia Fech." className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-xs outline-none" value={cardForm.fechamento} onChange={e => setCardForm({...cardForm, fechamento: e.target.value})} />
                                <input type="number" placeholder="Dia Venc." className="w-full bg-slate-900 border border-slate-800 rounded-lg p-3 text-white text-xs outline-none" value={cardForm.vencimento} onChange={e => setCardForm({...cardForm, vencimento: e.target.value})} />
                            </div>
                            <button onClick={handleAddCard} className="w-full py-3 bg-purple-600 hover:bg-purple-500 text-white rounded-lg font-black uppercase text-xs">Salvar Cartão</button>
                        </div>

                        <div className="space-y-2">
                            {cards.map(card => (
                                <div key={card.id} className="flex justify-between items-center bg-slate-900 p-3 rounded-lg border border-slate-800">
                                    <div>
