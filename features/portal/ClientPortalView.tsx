    ...
    <div className="min-h-screen bg-slate-950 flex justify-center p-0 sm:p-4 overflow-hidden">
      ...
      {clientNotification && clientNotification.length > 0 && clientNotification[0].show && (
          <div className="fixed top-6 left-4 right-4 z-[200] animate-in slide-in-from-top-6 duration-500 pointer-events-none">
             <div className="bg-slate-900 text-white p-4 rounded-lg shadow-2xl border-l-4 border-rose-500 flex items-start gap-3 max-w-sm mx-auto pointer-events-auto ring-1 ring-white/10">
      ...
      <div className="w-full max-w-lg bg-slate-900 sm:rounded-lg flex flex-col h-full sm:h-[90vh] sm:border border-slate-800 shadow-2xl overflow-hidden relative">
        ...
        <div className="bg-slate-950 border-b border-slate-800 p-5 flex items-center justify-between shrink-0 relative z-10">
            ...
            <button onClick={() => window.location.href = '/'} className="p-2 bg-slate-900 border border-slate-800 text-slate-400 rounded-lg hover:text-white transition-colors">
                <LogOut size={18}/>
            </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-6 relative">
            ...
            <div className="space-y-4">
                ...
                {clientContracts.length === 0 ? (
                    <div className="text-center py-10 text-slate-600 text-xs font-bold uppercase border-2 border-dashed border-slate-800 rounded-lg">
            ...
            {clientContracts.length > 0 && (
                <div className="bg-slate-950 p-4 rounded-lg border border-slate-800/50 flex items-center gap-3 opacity-60">
                    <div className="p-2 bg-slate-800 rounded-lg text-slate-400"><Building size={16}/></div>
      ...
      {isLegalOpen && (
        <div className="fixed inset-0 bg-slate-950/95 flex items-center justify-center z-[150] p-4 backdrop-blur-sm">
          <div className="bg-slate-900 p-6 rounded-lg border border-indigo-500/30 max-w-lg w-full shadow-2xl animate-in zoom-in-95 relative">
                ...
                <div className="w-full space-y-3 mb-6 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                    {portalDocuments.length === 0 ? (
                        ...
                        portalDocuments.map(doc => (
                            <div key={doc.id} className="bg-slate-950/50 p-4 rounded-lg border border-slate-800 text-left flex justify-between items-center group hover:border-indigo-500/50 transition-all backdrop-blur-sm">
                                ...
                                <div className="flex gap-2 items-center">
                                    <button
                                        ...
                                        className="p-2.5 bg-slate-800 text-slate-400 rounded-lg hover:text-white hover:bg-slate-700 transition-all"
                                        title="Visualizar"
                                    >
                                    ...
                                    {doc.status_assinatura !== 'ASSINADO' ? (
                                        <button
                                            ...
                                            disabled={isSigning}
                                            className="px-4 py-2.5 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase hover:bg-indigo-500 shadow-lg shadow-indigo-600/20 transition-all disabled:opacity-50"
                                        >
                                    ...
                                    <button
                                        ...
                                        className="p-2.5 bg-slate-900/50 text-slate-600 rounded-lg hover:text-rose-500 hover:bg-rose-500/10 transition-all ml-1"
