    ...
    <Modal onClose={handleClose} title={result ? "Convite Gerado" : "Novo Membro"}>
      {!result ? (
        <div className="space-y-5">
          <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 flex items-center gap-3 text-slate-400 text-xs leading-relaxed">
          ...
          <button
            ...
            className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-black uppercase text-xs flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20 transition-all"
          >
        ...
        <div className="text-center space-y-6 animate-in zoom-in-95 duration-300">
          ...
          <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 relative group cursor-pointer" onClick={handleCopy}>
             <p className="text-blue-400 text-xs font-mono break-all line-clamp-2">{result.link}</p>
             <div className="absolute inset-0 bg-slate-900/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity rounded-lg backdrop-blur-sm">
                <span className="text-white font-bold text-xs flex items-center gap-2"><Copy size={14}/> Copiar Link</span>
             </div>
          </div>

          <div className="flex gap-3">
            <button onClick={handleCopy} className="flex-1 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-black uppercase text-xs flex items-center justify-center gap-2 shadow-lg transition-all">
                <Copy size={16}/> Copiar
            </button>
            <button onClick={handleClose} className="px-8 py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-lg font-black uppercase text-xs transition-all">
                Concluir
            </button>
          </div>
        </div>
