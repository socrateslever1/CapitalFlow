...
export const ContractDetailsPage: React.FC<ContractDetailsPageProps> = ({
    ...
    const openPortalFile = async (url?: string | null) => {
        ...
        if (authorizedUrl && fileWindow) fileWindow.location.href = authorizedUrl;
        else fileWindow?.close();
    };
        ...
        <div className="flex flex-col gap-3 animate-in fade-in duration-500 pb-24 md:pb-6">
            ...
            <div className="bg-slate-950/90 backdrop-blur-md py-3 -mx-4 px-4 border-b border-slate-800/50 flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full bg-gradient-to-br flex items-center justify-center text-white shrink-0 shadow-lg ${status === 'PAID' ? 'from-emerald-500 to-emerald-600 shadow-emerald-900/20' : status === 'OVERDUE' ? 'from-rose-500 to-rose-600 shadow-rose-900/20' : 'from-blue-500 to-blue-600 shadow-blue-900/20'}`}>
                        <FileText size={16} />
                    </div>
                    <div>
                        <div className="flex items-center gap-2 mb-0.5">
                            <h1 className="text-base font-semibold text-white uppercase tracking-tight">{loan.debtorName}</h1>
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-black text-white uppercase tracking-widest ${statusColor}`}>{status === 'OVERDUE' ? 'Atrasado' : status === 'PAID' ? 'Quitado' : 'Ativo'}</span>
                            {loan.last_billed_at && new Date(loan.last_billed_at).toDateString() === new Date().toDateString() && (
                                <span className="px-2 py-0.5 bg-emerald-500 rounded-full text-[9px] font-black text-white uppercase tracking-widest animate-in fade-in zoom-in duration-300">Cobrado Hoje</span>
                            )}
                        </div>
                        <div className="flex items-center gap-3">
                            <p className="text-[10px] text-slate-500 font-medium uppercase tracking-widest flex items-center gap-1"><User size={10}/> {loan.debtorPhone}</p>
                        </div>
                    </div>
                </div>

                <div className="grid grid-cols-2 md:flex items-center gap-3 md:gap-6">
                    <div className="space-y-0.5"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Valor Principal</p><p className="text-sm font-black text-white">{formatMoney(loan.principal, isStealthMode)}</p></div>
                    <div className="space-y-0.5 text-right md:text-left"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Próximo Vencimento</p><p className="text-sm font-black text-blue-400">{nextDueDateDisplay}</p></div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-6">
                    {loan.activeAgreement ? (
                        <div className="space-y-4">
                            <h3 className="text-xs font-black uppercase text-indigo-400 tracking-widest flex items-center gap-2"><History size={16} /> Acordo Ativo</h3>
                            <AgreementView agreement={loan.activeAgreement} loan={loan} activeUser={activeUser} onUpdate={onRefresh || (() => {})} onPayment={(inst, amount, forgiveLateFee) => onAgreementPayment?.(loan, loan.activeAgreement!, inst, amount, forgiveLateFee)} onReversePayment={(inst) => onReverseAgreementPayment?.(loan, loan.activeAgreement!, inst)} isStealthMode={isStealthMode} onNavigate={onNavigate} />
                        </div>
                    ) : (
                        <div className="bg-slate-900 border border-slate-800 rounded-lg p-6 space-y-6">
                            <h3 className="text-xs font-black uppercase text-slate-500 tracking-widest flex items-center gap-2"><TrendingUp size={16} className="text-blue-500"/> Resumo Financeiro</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Principal Restante</p><p className="text-xl font-black text-white">{formatMoney(debtBreakdown.principal, isStealthMode)}</p></div>
                                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Juros Acumulados</p><p className="text-xl font-black text-blue-400">{formatMoney(debtBreakdown.interest, isStealthMode)}</p></div>
                                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg"><p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Multa/Mora</p><p className="text-xl font-black text-rose-400">{formatMoney(debtBreakdown.fine + debtBreakdown.dailyMora, isStealthMode)}</p></div>
                                <div className="bg-slate-950 border border-slate-800 p-4 rounded-lg ring-2 ring-emerald-500/20"><p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-1">Total Atual</p><p className="text-xl font-black text-emerald-400">{formatMoney(debtBreakdown.total, isStealthMode)}</p></div>
                            </div>
                            {delayDetails && (
                                <div className="mt-6 pt-6 border-t border-slate-800 animate-in fade-in slide-in-from-top-2 duration-500">
                                    <div className="flex items-center justify-between mb-4"><h4 className="text-[10px] font-black uppercase text-rose-500 tracking-widest flex items-center gap-2"><AlertTriangle size={12} /> Detalhamento de Atraso</h4><span className="px-2 py-0.5 bg-rose-500/10 text-rose-500 text-[9px] font-bold rounded-full border border-rose-500/20">{delayDetails.totalMonths} {delayDetails.totalMonths === 1 ? 'Mês' : 'Meses'} em atraso</span></div>
                                    <div className="space-y-3">{delayDetails.items.map((item, idx) => <div key={idx} className="flex items-center justify-between p-3 bg-slate-950/50 border border-slate-800 rounded-lg group hover:border-rose-500/30 transition-all"><div className="flex flex-col"><span className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Parcela {item.number} • {formatBRDate(item.dueDate)}</span><span className="text-[11px] font-black text-white uppercase leading-none italic">Vencimento {formatBRDate(item.dueDate)}</span></div><div className="text-right"><p className="text-sm font-black text-rose-400">{formatMoney(item.total, isStealthMode)}</p><p className="text-[9px] font-bold text-slate-600 uppercase tracking-tighter">Prin: {formatMoney(item.principal, isStealthMode)} + Enc: {formatMoney(item.interest, isStealthMode)}</p></div></div>)}</div>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-lg p-6 space-y-4">
                        <h3 className="text-xs font-black uppercase text-slate-500 tracking-widest flex items-center gap-2"><ShieldCheck size={16} className="text-purple-500"/> Ações Rápidas</h3>
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            <button onClick={() => onOpenMessage(loan)} className="p-3 bg-slate-950 border border-slate-800 rounded-lg text-[9px] font-black uppercase text-slate-400 hover:text-emerald-400"><MessageSquare size={14} className="mx-auto mb-1"/> Cobrar</button>
                            <button onClick={() => onEdit(loan)} className="p-3 bg-slate-950 border border-slate-800 rounded-lg text-[9px] font-black uppercase text-slate-400 hover:text-blue-400"><FileEdit size={14} className="mx-auto mb-1"/> Editar</button>
                            <button onClick={() => onExportExtrato(loan)} className="p-3 bg-slate-950 border border-slate-800 rounded-lg text-[9px] font-black uppercase text-slate-400 hover:text-white"><Download size={14} className="mx-auto mb-1"/> Extrato</button>
                        </div>
                    </div>

                    {(clientPortalFiles.length > 0 || clientVisibleFiles.length > 0) && (
                ...
                <div className="space-y-6">
                    ...
                    <div className="md:hidden grid grid-cols-1 gap-4"><button onClick={() => onOpenMessage(loan)} className="flex items-center justify-center gap-2 p-4 bg-slate-900 border border-slate-800 rounded-lg text-[10px] font-black uppercase text-slate-400"><MessageSquare size={16}/> Cobrar</button></div>
                </div>
            </div>
        </div>
    );
