    ...
    <div className="space-y-6">
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-lg">
            <div className="flex flex-col sm:flex-row justify-end items-start sm:items-center gap-4 mb-6">
                <div className="bg-slate-950 border border-slate-800 p-2 rounded-lg flex items-center gap-2 w-full sm:w-auto">
            ...
            <div className="space-y-3">
                {allUsers.filter(u => (u.nome_operador || '').toLowerCase().includes(sacSearch.toLowerCase()) || (u.usuario_email || '').toLowerCase().includes(sacSearch.toLowerCase())).map(u => {
                    ...
                    return (
                        <div key={u.id} className="flex flex-col sm:flex-row items-center justify-between bg-slate-950 p-4 rounded-lg border border-slate-800 gap-4 group hover:border-slate-700 transition-all">
                            <div className="flex items-center gap-4 w-full sm:w-auto">
                                <div className={`w-12 h-12 rounded-lg flex items-center justify-center border-2 shrink-0 ${isAdmin ? 'bg-rose-500/10 border-rose-500/20 text-rose-500' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-0 border-slate-800 pt-3 sm:pt-0">
                                <button
                                    ...
                                    className="px-4 py-2 bg-slate-900 border border-slate-800 text-blue-400 hover:text-white hover:bg-blue-600 rounded-lg transition-all text-[10px] font-black uppercase flex items-center gap-2 flex-1 sm:flex-none justify-center"
                                >
                                ...
                                {isAdmin ? (
                                    <button
                                        ...
                                        className="p-2.5 bg-slate-900 border border-slate-800 text-slate-500 hover:text-rose-500 hover:border-rose-500/30 rounded-lg transition-all"
                                        title="Remover Admin"
                                    >
                                        <ShieldAlert size={16}/>
                                    </button>
                                    ...
                                    <button
                                        onClick={() => handleToggleAdmin(u)}
                                        className="p-2.5 bg-slate-900 border border-slate-800 text-slate-500 hover:text-emerald-500 hover:border-emerald-500/30 rounded-lg transition-all"
