...
export const PublicSignaturePage = () => {
    ...
    if (status === 'ERROR') return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-lg text-center max-w-md shadow-2xl">
                ...
                <button
                    ...
                    className="mt-6 px-6 py-3 bg-indigo-600 text-white rounded-lg font-black uppercase text-xs hover:bg-indigo-500 transition-all"
                >
                    Tentar Novamente
                </button>
            </div>
        ...
        <div className="min-h-screen bg-slate-950 flex flex-col lg:flex-row overflow-hidden">
                    ...
                    <div className="relative bg-white shadow-[0_20px_70px_rgba(0,0,0,0.5)] min-h-[1122px] rounded-sm overflow-hidden border border-white/10">
                        {status === 'SIGNING' && (
                            <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-md z-50 flex items-center justify-center">
                                <div className="bg-slate-900 p-10 rounded-lg shadow-2xl flex flex-col items-center gap-6 border border-white/10 animate-in zoom-in-95">
            ...
            <div className="w-full lg:w-[420px] bg-slate-900 border-t lg:border-l border-slate-800 p-6 flex flex-col shadow-2xl z-20 overflow-y-auto">
                <div className="mb-8 flex items-center gap-4">
                    <div className="p-4 bg-indigo-600/20 rounded-lg text-indigo-500 shadow-inner"><Gavel size={32}/></div>
                    <div>
                ...
                {status === 'SUCCESS' ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 animate-in zoom-in duration-500">
                        ...
                        <button onClick={handleExit} className="w-full py-5 bg-slate-800 text-white rounded-lg font-black uppercase text-xs hover:bg-slate-700 transition-all">Encerrar Sessão</button>
                    </div>
                ) : (
                    <div className="space-y-6 flex-1 flex flex-col">
                        <div className="bg-slate-950 p-5 rounded-lg border border-slate-800 shadow-inner">
                             ...
                             <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-900 border-2 border-slate-800 focus:border-indigo-500 rounded-lg px-4 py-3 text-white font-bold outline-none transition-all"
                        ...
                        <div className="mt-auto space-y-3">
                            {localError && (
                                <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg flex items-center gap-2 text-[10px] text-rose-400 font-bold uppercase animate-in fade-in slide-in-from-bottom-2">
                                    <AlertTriangle size={14}/> {localError}
                                </div>
                            )}

                            {isAlreadySigned ? (
                                <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center gap-3 text-emerald-500">
            ...
            {showTerms && (
                <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-lg p-8 space-y-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="bg-slate-950 p-6 rounded-lg border border-slate-800 h-64 overflow-y-auto custom-scrollbar text-xs text-slate-400 leading-relaxed space-y-4">
                        ...
                        <button onClick={() => setShowTerms(false)} className="w-full py-4 bg-indigo-600 text-white rounded-lg font-black uppercase text-xs">Entendido</button>
                    </div>
                </div>
            )}
        </div>
