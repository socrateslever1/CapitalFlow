-- Corrige montagem do array de papeis obrigatorios na assinatura publica.
-- PostgreSQL exige concatenacao com array explicito, nao texto solto.
...
begin
  ...
  if v_doc.snapshot ? 'debtorName' then
    v_required_roles := v_required_roles || array['DEBTOR'];
  end if;

  if v_doc.snapshot ? 'creditorName' then
    v_required_roles := v_required_roles || array['CREDITOR'];
  end if;
