...
DROP FUNCTION IF EXISTS public.portal_status_allows_access(text, boolean);
...
CREATE OR REPLACE FUNCTION public.portal_status_allows_access(p_status text, p_is_archived boolean DEFAULT false)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(p_is_archived, false) = false
    AND upper(COALESCE(p_status, '')) NOT IN (
      'ARQUIVADO', 'ARCHIVED',
      'CANCELADO', 'CANCELED',
      'DELETADO', 'DELETED',
      'EXCLUIDO', 'EXCLUÍDO'
...
GRANT EXECUTE ON FUNCTION public.portal_status_allows_access(text, boolean) TO anon, authenticated;
...
BEGIN
  ...
  WHERE l.portal_token = v_token_uuid
    ...
    AND public.portal_status_allows_access(l.status, l.is_archived)
  LIMIT 1;
