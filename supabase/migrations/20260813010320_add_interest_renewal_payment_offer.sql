...
ALTER TABLE public.parcelas
  ADD COLUMN IF NOT EXISTS payment_offer_type text NOT NULL DEFAULT 'SETTLEMENT';

ALTER TABLE public.parcelas
  DROP CONSTRAINT IF EXISTS parcelas_payment_offer_type_check,
  ADD CONSTRAINT parcelas_payment_offer_type_check
    CHECK (payment_offer_type IN ('SETTLEMENT', 'INTEREST_RENEWAL'));

ALTER TABLE public.installment_payment_offer_history
  ADD COLUMN IF NOT EXISTS offer_type text NOT NULL DEFAULT 'SETTLEMENT';

ALTER TABLE public.installment_payment_offer_history
  DROP CONSTRAINT IF EXISTS installment_payment_offer_history_offer_type_check,
  ADD CONSTRAINT installment_payment_offer_history_offer_type_check
    CHECK (offer_type IN ('SETTLEMENT', 'INTEREST_RENEWAL'));
...
BEGIN
  ...
  IF v_type = 'INTEREST_RENEWAL' THEN
    IF coalesce(p_discount_percent, 0) <> 0 OR coalesce(p_discount_value, 0) <> 0
       OR coalesce(p_waive_fine, false) OR coalesce(p_waive_daily_interest, false) THEN
      RAISE EXCEPTION 'Renovacao por juros exige o valor integral dos juros e encargos.';
    END IF;

    SELECT greatest(coalesce(p.interest_remaining, 0), 0),
           greatest(coalesce(p.late_fee_accrued, 0), 0)
    INTO v_interest, v_late_fee
  ...
  IF v_type = 'INTEREST_RENEWAL' THEN
    v_offer_amount := round(v_interest + v_late_fee, 2);
    UPDATE public.parcelas
    SET payment_offer_type = v_type,
        payment_offer_amount = v_offer_amount,
        payment_offer_gross_amount = v_offer_amount,
        payment_offer_original_amount = round(
          greatest(coalesce(principal_remaining, 0), 0) + v_offer_amount,
          2
        ),
        payment_offer_discount_percent = 0,
        payment_offer_discount_value = 0,
        payment_offer_discount_applied = 0,
        payment_offer_late_fee_forgiven = 0,
        payment_offer_waive_late_fee = false,
        payment_offer_waive_fine = false,
        payment_offer_waive_daily_interest = false,
        payment_offer_fine_forgiven = 0,
        payment_offer_daily_interest_forgiven = 0
    ...
    SET offer_type = v_type,
        ...
        late_fee_forgiven = 0,
        waive_fine = false,
        waive_daily_interest = false,
        fine_forgiven = 0,
        daily_interest_forgiven = 0,
...
BEGIN
  ...
  v_late_paid := round(greatest(coalesce(v_inst.late_fee_accrued, 0), 0), 2);
  ...
  PERFORM public.process_payment_v3_selective(
    ...
    0, v_interest_paid, v_late_paid, 0, 0, p_payment_date, false,
  ...
  -- Instalacoes atuais ja possuem um gatilho que renova contratos mensais
  -- ao registrar a transacao de juros. Reutilize o resultado para nao avancar
  -- a data nem incrementar renewal_count duas vezes.
  IF coalesce(v_post_payment.renewal_count, 0) > coalesce(v_inst.renewal_count, 0)
    ...
    v_new_interest := round(
      greatest(coalesce(v_post_payment.principal_remaining, 0), 0) * (v_rate / 100),
      2
    );
