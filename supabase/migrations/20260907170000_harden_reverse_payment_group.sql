...
BEGIN
  v_auth_uid := auth.uid();
  v_base_key := regexp_replace(coalesce(nullif(trim(p_idempotency_key), ''), ''), '(_lucro|_profit|-OVERPAY)$', '', 'i');
  ...
  IF NOT EXISTS (
    ...
    WHERE p.id = p_profile_id
      AND (p.user_id = v_auth_uid OR p.id = v_auth_uid OR p.dono_id = v_auth_uid OR p.supervisor_id = v_auth_uid OR p.owner_profile_id = v_auth_uid)
  ) THEN
  ...
  CREATE TEMP TABLE IF NOT EXISTS pg_temp.cf_reverse_targets ON COMMIT DROP AS SELECT * FROM public.transacoes WHERE false;
  ...
  WHERE t.profile_id = p_profile_id
    ...
    AND (
      ...
      OR t.idempotency_key = v_base_key || '_profit'
      OR t.idempotency_key = v_base_key || '-OVERPAY'
      OR regexp_replace(coalesce(t.idempotency_key, ''), '(_lucro|_profit|-OVERPAY)$', '', 'i') = v_base_key
    );
  ...
  SELECT loan_id INTO v_contract_id FROM pg_temp.cf_reverse_targets WHERE loan_id IS NOT NULL LIMIT 1;
  IF v_contract_id IS NOT NULL THEN
    SELECT status INTO v_contract_status FROM public.contratos WHERE id = v_contract_id FOR UPDATE;
  END IF;
  ...
  FOR v_tx IN SELECT * FROM pg_temp.cf_reverse_targets ORDER BY created_at NULLS FIRST, date NULLS FIRST, id LOOP
    IF v_tx.source_id IS NOT NULL THEN
      ...
      WHERE id = v_tx.source_id AND profile_id = p_profile_id;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Fonte % nao encontrada para estorno.', v_tx.source_id;
      END IF;
    END IF;

    INSERT INTO public.transacoes (
      id, profile_id, loan_id, installment_id, source_id, date, type, amount,
      principal_delta, interest_delta, late_fee_delta, notes, category, idempotency_key,
      operator_id, original_tx_id, reversed_of_transaction_id, meta, created_at,
      edited_at, edited_by, edit_reason, description
    ) VALUES (
      gen_random_uuid(), v_tx.profile_id, v_tx.loan_id, v_tx.installment_id, v_tx.source_id, v_now,
      'ESTORNO', round((-coalesce(v_tx.amount, 0))::numeric, 2),
      ...
      'ESTORNO', v_base_key || '_estorno_' || left(v_tx.id::text, 8), v_operator_id,
      v_tx.id, v_tx.id,
      jsonb_build_object(
        ...
        'contract_status_before_reversal', v_contract_status,
        'reversed_at', v_now
      ),
      v_now, v_now, v_operator_id, coalesce(nullif(trim(p_reason), ''), 'Estorno manual'),
  ...
  LOOP
    ...
    SET
      ...
      paid_date = CASE WHEN greatest(0, coalesce(p.paid_total, 0) - coalesce(v_inst.amount_sum, 0)) <= 0.005 THEN NULL ELSE p.paid_date END,
      last_payment_date = CASE WHEN greatest(0, coalesce(p.paid_total, 0) - coalesce(v_inst.amount_sum, 0)) <= 0.005 THEN NULL ELSE p.last_payment_date END,
      status = CASE
        WHEN (greatest(0, coalesce(p.principal_remaining, 0) + coalesce(v_inst.principal_sum, 0)) + greatest(0, coalesce(p.interest_remaining, 0) + coalesce(v_inst.interest_sum, 0)) + greatest(0, coalesce(p.late_fee_accrued, 0) + coalesce(v_inst.late_fee_sum, 0))) <= 0.005 THEN 'PAID'
      ...
      logs = coalesce(p.logs, '[]'::jsonb) || jsonb_build_array(jsonb_build_object(
        'at', v_now, 'type', 'PAYMENT_REVERSAL', 'idempotency_key', v_base_key,
        'amount', coalesce(v_inst.amount_sum, 0), 'reason', coalesce(nullif(trim(p_reason), ''), 'Estorno manual'),
        'operator_id', v_operator_id
      ))
    WHERE p.id = v_inst.installment_id AND p.profile_id = p_profile_id;
  END LOOP;
  ...
  IF v_contract_id IS NOT NULL THEN
    ...
    SET status = CASE
      WHEN EXISTS (
        ...
        WHERE p.loan_id = c.id AND p.profile_id = p_profile_id
          AND (coalesce(p.principal_remaining, 0) + coalesce(p.interest_remaining, 0) + coalesce(p.late_fee_accrued, 0)) > 0.005
      ) THEN CASE
        WHEN upper(coalesce(v_contract_status, '')) = 'EM_ACORDO' THEN 'EM_ACORDO'
        WHEN upper(coalesce(v_contract_status, '')) = 'ATRASADO' THEN 'ATRASADO'
        WHEN upper(coalesce(v_contract_status, '')) = 'EM_DIA' THEN 'EM_DIA'
        WHEN upper(coalesce(v_contract_status, '')) = 'ATIVO' THEN 'ATIVO'
        ELSE 'PENDING'
      END
      ELSE 'PAID'
    END
    WHERE c.id = v_contract_id AND (c.owner_id = p_profile_id OR c.profile_id = p_profile_id);
  END IF;

  UPDATE public.payment_transactions pt
  SET status = 'REVERSED'
  WHERE pt.idempotency_key::text = v_base_key AND pt.contract_id = v_contract_id;

  RETURN jsonb_build_object(
    'ok', true, 'idempotency_key', v_base_key, 'transactions_reversed', v_reversed_count,
    'amount', round(v_reversed_amount::numeric, 2), 'principal', round(v_reversed_principal::numeric, 2),
    'interest', round(v_reversed_interest::numeric, 2), 'late_fee', round(v_reversed_late_fee::numeric, 2),
