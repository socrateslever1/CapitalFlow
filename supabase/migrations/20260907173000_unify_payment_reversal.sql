...
DECLARE
  ...
  v_contract_id uuid;
  v_contract_previous_status text;
BEGIN
  v_auth_uid := auth.uid();
  v_base_key := regexp_replace(
    coalesce(nullif(trim(p_idempotency_key), ''), ''),
    '(_lucro|_profit|-OVERPAY)$',
    '',
    'i'
  );
  ...
  SELECT * FROM public.transacoes WHERE false;
  ...
  IF v_contract_id IS NOT NULL THEN
    SELECT status INTO v_contract_previous_status
    FROM public.contratos
    WHERE id = v_contract_id
      AND coalesce(owner_id, profile_id) = p_profile_id
    FOR UPDATE;
  END IF;
  ...
  FOR v_tx IN SELECT * FROM pg_temp.cf_reverse_targets ORDER BY created_at NULLS FIRST, date NULLS FIRST, id LOOP
    ...
    ELSEIF coalesce(v_tx.interest_delta,0) <> 0 OR coalesce(v_tx.late_fee_delta,0) <> 0 THEN
      UPDATE public.perfis
      SET interest_balance = round((coalesce(interest_balance,0) - coalesce(v_tx.interest_delta,0) - coalesce(v_tx.late_fee_delta,0))::numeric, 2)
    ...
    INSERT INTO public.transacoes (
      ...
      meta, created_at, edited_at, edited_by, edit_reason, description
    ) VALUES (
      gen_random_uuid(), v_tx.profile_id, v_tx.loan_id, v_tx.installment_id, v_tx.source_id,
      v_now, 'ESTORNO', round((-coalesce(v_tx.amount, 0))::numeric, 2),
  ...
  IF v_contract_id IS NOT NULL THEN
    ...
    SET status = CASE
      WHEN EXISTS (
        ...
        WHEN upper(coalesce(v_contract_previous_status,'')) IN ('EM_ACORDO','RENEGOCIADO') THEN v_contract_previous_status
        ELSE 'ATIVO'
      END
      ELSE 'PAID'
    END
    WHERE c.id = v_contract_id
      AND coalesce(c.owner_id, c.profile_id) = p_profile_id;
  END IF;
